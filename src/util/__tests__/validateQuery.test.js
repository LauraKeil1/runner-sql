...
it("query is valid (without ';')", () => {
  expect.assertions(1);
  return validateQuery(collectibleValidWithoutResultsSameResults, collectibleValidWithoutResultsSameResults.query2, "query2").then((collectible) => {
...
it("query is valid (with ';')", () => {
  ...
  return validateQuery(collectibleValidWithoutResultsDifferentResults, collectibleValidWithoutResultsDifferentResults.query2, "query2").then(
    (collectible) => {
      expect(collectible).toEqual(collectibleValidWithoutResultsDifferentResults);
    }
  );
});
